...
![Sanity CMS](https://img.shields.io/badge/Sanity_CMS-v6-F03E2F?logo=sanity&logoColor=white)
...
| [Sanity CMS](https://www.sanity.io)                       | v6      | Content management                |
...
app/
  ...
  api/draft-mode/         # Enable/disable routes for the Presentation Tool's draft preview
...
components/
  ...
  shared/                 # Logo, icon helpers, shared social rendering, disable-draft-mode button
...
sanity/
  ...
  lib/                    # Sanity utilities (client, cache-tags, image builder, live.ts for draft/live fetch)
  presentation/           # Presentation Tool document location resolvers
...
- **Presentation Tool for draft preview** — `sanity/lib/live.ts` (`defineLive`) powers draft-aware, real-time fetching; `sanity/presentation/resolve.ts` maps document types to frontend routes so editors can preview and click-to-edit from `/cms`'s Presentation tab. Coexists with, doesn't replace, the webhook-based ISR above. See `CLAUDE.md`'s Presentation Tool / Visual Editing section.
...
| `SANITY_API_READ_TOKEN`           | Yes      | Viewer-role Sanity API token (server-only). Required for the Presentation Tool / draft mode preview.       |
...
7. Create a Viewer-role API token at [manage.sanity.io](https://manage.sanity.io) → your project → API → Tokens, and set it as `SANITY_API_READ_TOKEN` to enable the Presentation Tool's draft preview. Also add your deployed domain as a CORS origin (with credentials) under API → CORS Origins.
