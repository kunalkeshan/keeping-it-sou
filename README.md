...
- **Config-driven URL** — `config/site.ts` is the single source of truth for the site URL, falling back from `VERCEL_PROJECT_PRODUCTION_URL` to `SITE_URL`. Never hardcode the origin elsewhere.
...
| `SITE_URL`                        | Yes      | Canonical site URL (e.g. `https://keepingitsou.com`)             |
...
> **Note:** On Vercel, `VERCEL_PROJECT_PRODUCTION_URL` is set automatically and takes priority over `SITE_URL` for the canonical URL (see `config/site.ts`).
